		...
		e.prototype = {
			...
			checkMK: function () {
				if (this.options.frve) {
					...
					if (0 > a.text().indexOf(this.options.frvem) || 0 > c.length) a.append("<a style='text-decoration:none;' target='_blank' href='" + this.options.freelink + "' ><div style='display:none;visibility:visible;position:absolute;top:10px;left:10px;width:170px;height:18px;line-height:18px;text-align:center;border-radius:5px;-moz-border-radius:5px;-webkit-border-radius:5px;color:#333;font-size:0px;font-family:Arial,Helvetica,sans-serif;'>" +
						...
						style: (c.attr("style") || "") + "display:block!important;visibility:visible!important;font-size:0px!important;"
					}), a.attr({
						style: a.attr("style") + "display:block!important;visibility:visible!important;font-size:0px!important;"
					})
				}
			},
			initData: function (a) {
			...
			slideRun: function (a, c, d, e) {
				...
				if (!(0 > a)) {
					...
					(5 == a || 6 == a || 7 == a || 8 == a || 9 == a || 10 == a || 11 == a) && !d && this.elemArray[this.curElem][12] ? (this.showingPoster = !0, this.showPoster()) :
						(b(".html5gallery-video-play-" + this.id, l).length && b(".html5gallery-video-play-" + this.id, l).remove(), 1 == a ? this.showImage() : (this.options.hidetitlewhenvideoisplaying && b(".html5gallery-title-" + this.id, l).hide(), 5 == a || 6 == a || 7 == a || 8 == a ? this.showVideo(d, e) : 9 == a ? this.showYoutube(d) : 10 == a ? this.showVimeo(d) : 11 == a ? this.showEmbedVideo(d) : 2 == a ? this.showSWF() : 12 == a && this.showIframe()));
					this.checkMK();
			...
			showYoutube: function (a) {
				...
				if (this.isFullscreen) {
					var d = this.elemArray[this.curElem][17] ? Math.min(this.elemArray[this.curElem][17], this.fullscreenWidth) : Math.min(d, this.fullscreenWidth),
						...
						Math.min(this.elemArray[this.curElem][18], this.fullscreenHeight) : Math.min(e, this.fullscreenHeight);
					this.adjustFullscreen(d, f);
					e = d;
					d = f
				} else b(".html5gallery-elem-" + this.id, this.$gallery).css({
				...
				f = this.elemArray[this.curElem][2];
				b(".html5gallery-elem-" + this.id, c).html("<div class='html5gallery-loading-center-" + this.id + "'></div><div id='html5gallery-elem-video-" + this.id + "' style='display:block;position:absolute;overflow:hidden;top:" +
					(this.options.slideshadow && !this.isFullscreen ? 4 : 0) + "px;left:" + (this.options.slideshadow && !this.isFullscreen ? 4 : 0) + "px;width:" + e + "px;height:" + d + "px;'></div>" + this.options.watermarkcode);
			...
			showVimeo: function (a) {
				...
				j = 0 > j.indexOf("?") ? j + "?" : j + "&";
				j = a && !this.options.isAndroid ?
					j + "autoplay=1" : j + "autoplay=0";
				...
				b(".html5gallery-elem-video-" +
					this.id, d).html("<iframe id='html5gallery_vimeo_" + this.id + "' width='" + f + "' height='" + e + "' src='" + j + "' frameborder='0' allow='autoplay' webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>");
			...
			showIframe: function () {
				...
				b(".html5gallery-elem-" + this.id, a).html("<div class='html5gallery-loading-center-" + this.id + "'></div><div class='html5gallery-elem-iframe-" + this.id + "' style='display:block;position:absolute;overflow:hidden;top:" + (this.options.slideshadow && !this.isFullscreen ? 4 : 0) + "px;left:" + (this.options.slideshadow &&
					!this.isFullscreen ? 4 : 0) + "px;width:" + d + "px;height:" + c + "px;'></div>" + this.options.watermarkcode);
			...
			checkType: function (a) {
				return !a ? -1 : a.match(/\.(jpg|gif|png|bmp|jpeg)(.*)?$/i) ?
					1 : a.match(/[^\.]\.(swf)\s*$/i) ? 2 : a.match(/[^\.]\.(mp3)\s*$/i) ? 3 : a.match(/[^\.]\.(pdf)\s*$/i) ? 4 : a.match(/\.(flv)(.*)?$/i) ? 5 : a.match(/\.(mp4|m4v)(.*)?$/i) ? 6 : a.match(/\.(ogv|ogg)(.*)?$/i) ? 7 : a.match(/\.(webm)(.*)?$/i) ? 8 : a.match(/\:\/\/.*(youtube\.com)/i) || a.match(/\:\/\/.*(youtu\.be)/i) || a.match(/\:\/\/.*(youtube-nocookie\.com)/i) ? 9 : a.match(/\:\/\/.*(vimeo\.com)/i) ? 10 : a.match(/\:\/\/.*(dailymotion\.com)/i) || a.match(/\:\/\/.*(dai\.ly)/i) ? 11 : 1
			},
			onChange: function () {
			...
			embedHTML5Video: function (a, c, d, e, f, j, l, g) {
				...
				a.html("<div class='html5gallery-elem-video-container-" + this.id + "' style='position:relative;display:block;background-color:#000;width:100%;height:100%;'><video id='html5gallery-elem-html5-video-" +
					this.id + "' width='100%' height='100%'" + (this.options.nativehtml5controls && !this.options.videohidecontrols ? " controls" : "") + (this.options.html5videonodownload ? ' controlsList="nodownload"' : "") + (this.options.mutevideo ? " muted" : "") + "></div>");
				...
				!this.options.nativehtml5controls && !this.options.videohidecontrols && (b("video", a).data("src", e), b("video", a).data("hd", f), b("video", a).data("ishd", this.isHd), b("video", a).addHTML5VideoControls(this.options.skinfolder,
					this, "#html5gallery-elem-html5-video-" + this.id, !1, !1, this.options.defaultvideovolume, this.options.fullscreennativecontrols, this.options.html5videonodownload, null));
			...
			embedFlash: function (a, b, d, e, f, j, k) {
				...
				} else a.html("<div class='html5gallery-elem-error-" +
		...
		this.each(function () {
			...
			if ((!b(this).data("donotinit") || k && k.forceinit) && !b(this).data("inited")) {
				...
				var h = {
					...
					z = "vertical" == this.options.skin ? b.extend(z, t) : "verticallight" == this.options.skin ? b.extend(z, s) : "showcase" == this.options.skin ? b.extend(z, x) : "light" ==
				...
				this.options.lightbox &&
					(this.options.showfullscreenbutton = !1);
				...
				if (this.options.screenquery)
					for (c in this.options.screenquery) h <= this.options.screenquery[c].screenwidth && (this.options.screenquery[c].gallerywidth && (this.options.width = this.options.screenquery[c].gallerywidth), this.options.screenquery[c].galleryheight && (this.options.height = this.options.screenquery[c].galleryheight), this.options.screenquery[c].thumbwidth &&
